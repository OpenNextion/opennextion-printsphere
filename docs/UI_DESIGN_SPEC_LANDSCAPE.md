...
| card name | card 内 `x=12,y=10,w=208,h=24` | `18/#F7FAFC/bold` | 单行省略 |
...
| detail title | `x=336,y=60,w=120,h=40` | `20/#F7FAFC/bold` | 最多 2 行；raw serial 禁止 |
...
| 2026-06-06 复验 Main 标题仍有方框 | fallback 检测不够硬 | 仅检测 CJK codepoint 不足；横屏 demo 第一版必须对用户文本统一 ASCII-safe |
...
后续优先方案：引入小型 CJK 字体子集，用于任务名、Cover title、Printer display name 等真实用户输入文本。字体子集应只覆盖 UI 需要的简体中文常用字、ASCII、数字、常见标点和符号，避免为 demo-first 阶段引入全量大字体。字体资产方案由 UI Implementation 线程另行评估内存占用后回报主线程确认。

横屏 demo 第一版强制采用 ASCII-safe display policy，目标是“绝不显示方框”，不是显示中文：

- 不修改 `PrinterSnapshot` 或协议原始字符串，只在 label display helper 中处理。
- 对用户可命名文本，只要原始 UTF-8 字符串中存在任意非 ASCII byte/codepoint，就不允许把原始非 ASCII 字符送入 LVGL label。
- display helper 必须逐字节输出 `0x20..0x7E` 可见 ASCII；其他 byte/codepoint 全部过滤或替换为空格，然后 trim/collapse spaces。
- 如果过滤后仍有至少 3 个可见 ASCII 字符，显示 ASCII fragment，并追加 `[UTF8]`；例如 `OpenNextion3.5 [UTF8]`。如果后续实现能可靠识别 CJK，可使用 `[CJK]`，但不得因此放宽非 ASCII 过滤。
- 如果过滤后不足 3 个可见 ASCII 字符，显示语义 fallback：任务/Cover 用 `Project name [UTF8]`，Printer name 用 `Printer name [UTF8]`。
- ASCII fragment 超过 label 宽度时仍按本规范 DOT/WRAP 处理；helper 层可先缩短为 `prefix...suffix [UTF8]`，但不得把非 ASCII 字符留在 prefix/suffix。
- serial、IP、温度、进度、层数、PIN 等机器字段不走 CJK fallback，按原 ASCII 显示。
- 真实中文显示作为后续 CJK font slice；在该 slice 通过主线程确认前，横屏 demo 任何用户标题、文件名、打印机名都必须 ASCII-safe。

必须纳入 ASCII-safe 策略的文本源：

| UI 文本 | 现有来源 | 横屏显示要求 |
|---|---|---|
| Cover title | `preview_subnote_text(snapshot)`：`job_name`、`preview_title`、`cloud_detail`、`preview_hint` | `x=276,y=60,w=180,h=72`，最多 2 行优先，不得显示任何非 ASCII 原字符 |
| Main job title | `job_name`，fallback `gcode_file`、`preview_title` 待实现线程按现有代码确认 | `x=220,y=48,w=248,h=38`，最多 2 行，ASCII-safe 后不得遮挡 layer |
| Printer card name | `Ui::PrinterCardInfo.name`、`model`、cloud fallback 名称 | card 内 `x=12,y=10,w=208,h=24`，单行 DOT |
...
| Setup SSID | `setup_access_text(snapshot)` 中 `setup_ap_ssid` | SSID 也要 ASCII-safe；`PW` 与 `Open <ip>` 必须完整可读 |
...
|---|---|---|---|
| `preview_blob` 已解码且 image visible | 显示 contain image | 显示 job/preview title；ASCII-safe 后最多 2 行优先 | 不显示 `Loading cloud cover`；只显示 source/material/短 cloud detail |
...
|---|---|
| topbar 标题 | `DOT`，1 行 |
| 任务名 | 横屏 demo 必须 ASCII-safe；Main/Cover 最多 2 行；Main 空闲态优先单行 `DOT`；超出省略 |
...
| printer title/name | 优先 nickname/model；raw serial 不做标题；横屏 demo 必须 ASCII-safe |
| printer host/model/serial | `DOT`，1 行；serial 使用 `SN <head6>...<tail4>` 或 model + tail |
| material name | `DOT`，1 行；材料类型必须保留 |
| Cover title | ASCII-safe 后最多 2 行优先；不得显示 tofu boxes；不得遮挡 detail |
...
| L5 Cover | Cover 左图右文、ASCII-safe/fallback、loading/detail 优先级 | UI preview image bounds/text | `preview_blob` contain；image visible 后 loading 消失；不显示方框 |
...
| L7 Cloud text + portal regression | ASCII-safe 用户文本、H2C cloud serial、Cover title、Web Config/AP/IP 可发现性 | UI text/layout only | Main/Cover/Printer 不显示方框；长 serial 不撑爆；Printer/Main 可持续看到 Web Config 入口 |
...
- no-op 边界。
- H2C Cloud demo 文本策略：横屏用户文本强制 ASCII-safe；cloud serial shortened；Printer 页 `92 px` 卡片和拆分 IP/PIN；Cover image visible 后隐藏 loading；横屏 Printer/Main 可发现 Web Config IP/PIN。
...
- 小圆弧进度样式。
- 扩展 MDI glyph。
- CJK 字体子集或全量 CJK 字库；横屏 demo 第一版先以 ASCII-safe 保证无方框。
...
- H2C Cloud 场景下 Main job title、Cover title、Printer display name 不得显示任何 tofu 方框；横屏 demo 第一版遇到任意非 ASCII 用户文本必须输出 ASCII-safe fragment + `[UTF8]` 或 semantic fallback。
